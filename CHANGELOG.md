...
- Add ability to specify `USpatialMultiWorkerSettings` class from command line. Specifying a `SoftClassPath` via `-multiWorkerSettingsClass=MultiWorkerSettingsClassName` will infer `bMultiWorkerEnabled=true` and `MultiWorkerSettingsClassName`.
