...
void FSpatialGDKPackageAssembly::BuildAssembly(const FString& TargetName, const FString& Platform, const FString& Configuration, const FString& AdditionalArgs)
{
...
bool FSpatialGDKPackageAssembly::NextStep()
{
	...
	if (Steps.Dequeue(Target))
	{
		...
		case EPackageAssemblyStep::BUILD_CLIENT:
			AsyncTask(ENamedThreads::GameThread, [this]()
			{
				BuildAssembly(FApp::GetProjectName(), Win64Platform, CloudDeploymentConfiguration.BuildConfiguration, CloudDeploymentConfiguration.BuildClientExtraArgs + GetExtraArgsForClientTarget());
			});
			break;
		case EPackageAssemblyStep::BUILD_SIMULATED_PLAYERS:
			AsyncTask(ENamedThreads::GameThread, [this]()
			{
				BuildAssembly(FString::Printf(TEXT("%sSimulatedPlayer"), FApp::GetProjectName()), LinuxPlatform, CloudDeploymentConfiguration.BuildConfiguration, CloudDeploymentConfiguration.BuildSimulatedPlayerExtraArgs + GetExtraArgsForClientTarget());
			});
			break;
